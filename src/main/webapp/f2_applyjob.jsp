<%-- 
    Document   : f2_applyjob
    Created on : Nov 9, 2023, 1:21:35 AM
    Author     : Admin
--%>
...
<html lang="en">
    ...
    <body>
        ...
        <div class="container_love">
            <div class="all_love">
                <h1 class="title">Việc làm đã ứng tuyển</h1>

                <c:forEach items="${hisList}" var="his">
                    <c:if test="${his.idUser == sessionScope.account.idUser}">
                        <c:forEach items="${jobList}" var="job">
                            <c:if test="${his.idJob == job.idJob}">
                                            ...
                                            <c:if test="${user.email == em.emailEmployer}">
                                                <div class="love">
                                                    ...
                                                    <div class="love_delete">
                                                        <c:if test="${his.status == 'SUCCESSFULL'}">
                                                            <p style="color: green; font-weight: bolder">SUCCESSFULL</p>
                                                        </c:if>
                                                        <c:if test="${his.status == 'PROCESSING'}">
                                                            <p style="color: #EB6304; font-weight: bolder">PROCESSING</p>
                                                        </c:if>
                                                        <c:if test="${his.status == 'FAILED'}">
                                                            <p style="color: red; font-weight: bolder">FAILED</p>
                                                        </c:if>
                                                    </div>
                                                </div>
